...
// final variant: every desktop and mobile frame plus the state variants,
// laid out in labelled rows; row offsets are computed from the dumps themselves
// usage: node generate-fig-all.js <out.fig>   (zrzuty: bash tools/dump-all.sh)
const fs = require('fs');
...
const outFig = process.argv[2] || 'litro-final.fig';
...
nodeChanges.push({
  guid: { sessionID: 0, localID: 1 }, phase: 'CREATED', type: 'CANVAS', name: 'LITRO — Desktop + Mobile',
  parentIndex: { guid: { sessionID: 0, localID: 0 }, position: '!' },
  visible: true, opacity: 1, transform: IDENT, strokeWeight: 0, strokeAlign: 'CENTER', strokeJoin: 'BEVEL',
  backgroundColor: { r: 0.949, g: 0.945, b: 0.941, a: 1 }, backgroundOpacity: 1, backgroundEnabled: true,
});

// Rândurile din fișierul final. Numele ramei = ce vede designerul în Figma.
const ROWS = [
  ['Desktop · flux principal (RO)', [
    ['home-c', 'Desktop / Home C'], ['home-d', 'Desktop / Home D'], ['listing-c', 'Desktop / Listing C'],
    ['hotel', 'Desktop / Hotel Details'], ['checkout', 'Desktop / Checkout'], ['thankyou', 'Desktop / Thank you'],
  ]],
  ['Desktop · main flow (EN)', [
    ['home-c-en', 'Desktop EN / Home C'], ['home-d-en', 'Desktop EN / Home D'], ['listing-c-en', 'Desktop EN / Listing C'],
    ['hotel-en', 'Desktop EN / Hotel Details'], ['checkout-en', 'Desktop EN / Checkout'], ['thankyou-en', 'Desktop EN / Thank you'],
  ]],
  ['Desktop · stări', [
    ['listing-c-denb', 'Desktop / Listing — celule compacte (B)'],
    ['listing-c-denc', 'Desktop / Listing — celule cu iconițe (C)'],
    ['listing-c-member', 'Desktop / Listing — membru FRIENDS (autentificat)'],
    ['home-c-member', 'Desktop / Home C — membru FRIENDS (autentificat)'],
  ]],
  ['Desktop · arhivă A / B', [
    ['home-a', 'Desktop / Home A (arhivă)'], ['home-b', 'Desktop / Home B (arhivă)'],
    ['listing-a', 'Desktop / Listing A (arhivă)'], ['listing-b', 'Desktop / Listing B (arhivă)'],
  ]],
  ['Mobile · flux principal (RO)', [
    ['m-home', 'Mobile / Home'], ['m-listing', 'Mobile / Listing'], ['m-hotel', 'Mobile / Hotel Details'],
    ['m-checkout', 'Mobile / Checkout'], ['m-thankyou', 'Mobile / Thank you'],
  ]],
  ['Mobile · main flow (EN)', [
    ['m-home-en', 'Mobile EN / Home'], ['m-listing-en', 'Mobile EN / Listing'], ['m-hotel-en', 'Mobile EN / Hotel Details'],
    ['m-checkout-en', 'Mobile EN / Checkout'], ['m-thankyou-en', 'Mobile EN / Thank you'],
  ]],
  ['Mobile · stări', [
    ['m-listing-denb', 'Mobile / Listing — celule compacte (B)'],
    ['m-listing-denc', 'Mobile / Listing — rezumat pe orizontală (C)'],
    ['m-listing-member', 'Mobile / Listing — membru FRIENDS (autentificat)'],
    ['m-listing-few', 'Mobile / Listing — inventar redus (banner de date + salvare)'],
    ['m-hotel-room', 'Mobile / Hotel — cameră aleasă (bara de rezervare)'],
  ]],
];

// Aranjare automată: fiecare rând începe de la x = 0, iar înălțimea rândului
// vine din cea mai înaltă ramă din el — nimic nu se suprapune.
const GAP_X = 200, GAP_Y = 400;
const frameBox = f => { const d = JSON.parse(fs.readFileSync(path.join(SCRATCH, f), 'utf8')); return d[0].rect; };
const pages = [];
const canvasExtent = { w: 0, h: 0 };
{
  ...
  for (const [label, items] of ROWS) {
    ...
    for (const [slug, name] of items) {
      const file = 'dump-' + slug + '.json';
      if (!fs.existsSync(path.join(SCRATCH, file))) { console.warn('lipsește', file, '— sar peste'); continue; }
      const r = frameBox(file);
      pages.push({ file, name, ox, oy });
      ox += r.w + GAP_X;
      if (r.h > rowH) rowH = r.h;
      if (ox - GAP_X > canvasExtent.w) canvasExtent.w = ox - GAP_X;
    }
    console.log(label, '→', ox ? Math.round(ox / 1000) + 'k px lățime' : 'gol');
    oy += rowH + GAP_Y;
...
const meta = {
  ...
  file_name: 'LITRO — Desktop + Mobile (final)',
...
try {
  execSync(`sips -Z 400 "/Users/mkoslacz/Workspaces/claude/litoralul_nop_designs/desktop-redesign/preview-home-c.png" --out "${stage}/thumbnail.png" >/dev/null 2>&1`);
